...
import json
import math
from .utils import extract_scale_zero_point, extract_val_scale_zero_point, value_info_dims, rcc_to_dict, save_to_file, find_simd_pe, get_dest_node, get_source_node, decompose_float, calc_bit_width_fixed
...
class NetworkHLS():
    def __init__(self, dtype=None, input_mem_width=None, output_mem_width=None, target_fps=30, intermediate_streams_depth=2):
        ...
        self.default_itermediate_streams_depth = intermediate_streams_depth
        # default
        cycle_time = 1/DefaultValues.freq_mhz
        if target_fps < 0:
            print("\033[91mWarning: Target FPS is negative setting target_latency to INF assuming fully rolled model.\033[0m")
            self.target_latency_s = math.inf
            self.target_latency_cc = math.inf
        else:
            self.target_latency_s = 1/target_fps
            self.target_latency_cc = math.ceil(self.target_latency_s/cycle_time)
        ...
        if self.dtype is Dtypes.FLOAT:
            print("\033[91mWarning: Using float type. Bit widths are 32 for data and 32 for integer part.\033[0m")
            ...
            self.default_input_mem_width = input_mem_width
            self.default_output_mem_width = output_mem_width
        ...
        if self.dtype is Dtypes.INT:
            ...
            self._act_min_value = "ACT_MIN_VALUE"
            self._act_max_value = "ACT_MAX_VALUE"
            self._scale_bit_width = "SCALE_BIT_WIDTH"
        ...
        if self.dtype is Dtypes.INT:
            # These will be set later on py the onnx_to_networkx function, they are not used in HLS but in the TB to quantize the input
            self.input_scale = 1
    ...
    def networkx_to_png(self, nx_graph, safix='', extra_node_label=None):
        ...
        if len(self.subgraphs) == 0:
            if extra_node_label is not None:
                for node in nx_graph.nodes():
                    ...
                    pydot_graph.add_node(pydot.Node(node.name, label=label))
            else:
                for node in nx_graph.nodes():
                    name = node.name
                    ...
                    pydot_graph.add_node(pydot.Node(node.name, label=label_1+label_2))
        else:
        ...
        for edge in nx_graph.edges():
            ...
            pydot_graph.add_edge(pydot.Edge(edge[0].name, edge[1].name, label=f"{depth}\n {shape}\n {mapping}", color="black"))
    ...
    def merge_several_nodes(self, nx_graph):
        ...
        for node in nx_graph.nodes():
            ...
            if (successor_of_successor is not None) and any(i in node.op_type for i in [SupportedNodes.conv, SupportedNodes.tr_conv, SupportedNodes.linear]) and any(i in successor.op_type for i in [SupportedNodes.bn, SupportedNodes.ins]) and any(i in successor_of_successor.op_type for i in [SupportedNodes.relu, SupportedNodes.relu6]):
                ...
                edge_attributes['depth'] = self.default_itermediate_streams_depth
    ...
    def add_sdwc_nodes(self, nx_graph):
        ''' This function adds StreamingDataWidthConverter_Batch nodes to the graph between nodes with different input/output stream widths
            If it faces a split node with successors that have the same input stream width, it will add a SDWC node between the split node and the predecessor of the split node
        '''
        print("Adding SDWC nodes...")
        def create_sdwc_node(node, succ):
            node_attributes = {}
            node_attributes['op_type'] = SupportedNodes.sdwc
            node_attributes['name'] = f"{SupportedNodes.sdwc}_{self.cnt_sdwc}"
            node_attributes['dtype'] = node.dtype
            node_attributes['pe'] = succ.simd
            node_attributes['simd'] = node.pe
            node_attributes['in_bits'] = node.out_bits
            node_attributes['in_int_bits'] = node.out_int_bits
            node_attributes['out_bits'] = succ.in_bits
            node_attributes['out_int_bits'] = succ.in_int_bits
            node_attributes['input_shape'] = node.output_shape
            node_attributes['output_shape'] = succ.input_shape
            node_attributes['_input_stream_width'] = node._output_stream_width
            node_attributes['_output_stream_width'] = succ._input_stream_width
            node_attributes['_input_accesses'] = node._output_accesses
            sdwc_node = NodeHLS(node_attributes)
            return sdwc_node
        ...
        for node in nx_graph.nodes():
            ...
            for succ in successors:
                if (node, succ) in skip_edges:
                    continue
                if succ.op_type == SupportedNodes.split:
                    split_node_successors = list(nx_graph.successors(succ))
                    successor_input_stream_widths = [succ.input_stream_width for succ in split_node_successors]
                    if len(set(successor_input_stream_widths)) == 1:
                        if node.output_stream_width != split_node_successors[0].input_stream_width:
                            sdwc_node = create_sdwc_node(node, split_node_successors[0])
                            edge_attributes = {'depth': nx_graph.edges[node, succ]['depth'], 'mapping': nx_graph.edges[node, succ]['mapping']}
                            nx_graph_tmp.add_node(sdwc_node)
                            add_nodes_list.append(sdwc_node)
                            add_edges_list.append((node, sdwc_node, edge_attributes))
                            for t_succ in successors:
                                add_edges_list.append((sdwc_node, t_succ, edge_attributes))
                                remove_edges_list.append((node, t_succ))
                            self.cnt_sdwc += 1
                            for split_succ in split_node_successors:
                                skip_edges.append((succ, split_succ))
                            continue
                if node.output_stream_width != succ.input_stream_width:
                    sdwc_node = create_sdwc_node(node, succ)
                    pre_sdwc_edge_attributes = {'depth': nx_graph.edges[node, succ]['depth'], 'mapping': nx_graph.edges[node, succ]['mapping']}
                    post_sdwc_edge_attributes = {'depth': self.default_itermediate_streams_depth, 'mapping': self.default_mapping}
                    nx_graph_tmp.add_node(sdwc_node)
                    add_nodes_list.append(sdwc_node)
                    add_edges_list.append((node, sdwc_node, pre_sdwc_edge_attributes))
                    add_edges_list.append((sdwc_node, succ, post_sdwc_edge_attributes))
                    remove_edges_list.append((node, succ))
                    self.cnt_sdwc += 1
    ...
    def onnx_to_networkx(self, model_path, brevitas, int_quant, scale_bit_width, stop_node=None, fixed_scale=False):
        ...
        Args:
            ...
            int_quant: Is the model quantized with Onnx QDQ quantization
            scale_bit_width: bit width for integer onnx quantization scale
        ...
        initilizers_dict = {}
        assert len(sess.get_inputs()) == 1, f'Only one input is supported, got {len(sess.get_inputs())}'  # To support this the logic has to be modifed to find the scale factor and zero point for each input so the HLS tb can quantize the input
        ...
        for i, initializer in enumerate(model.graph.initializer):
            ...
            if initilizers_dict[initializer.name].size >= 1e6:
                print(f"Error: {initializer.name} has a large size of {initilizers_dict[initializer.name].size}, this will cause issues with the HLS toolchain.")
                exit(-1)
        if int_quant or brevitas:
            input_quantizer = get_dest_node(sess.get_inputs()[0].name, model.graph)
            self.input_scale, self.input_zero_point = extract_scale_zero_point(input_quantizer, initilizers_dict)
        nx_graph = nx.DiGraph()
        processed_nodes = []
        for i, n in enumerate(model.graph.node):
            ...
            if n.op_type == 'Conv' or n.op_type == 'ConvTranspose':
                ...
                assert self.dtype is not Dtypes.INT, 'BatchNormalization layer is not supported with INT quantization'
                ...
                assert self.dtype is not Dtypes.INT, 'InstanceNormalization layer is not supported with INT quantization'
                ...
                node_attributes['out_int_bits'] = self.default_int_bits
                if self.dtype is Dtypes.INT:
                    tmp_node = get_source_node(n.input[0], model.graph)
                    inp1_scale, inp1_zero_point = extract_scale_zero_point(tmp_node, initilizers_dict)
                    node_attributes['input1_zero_point'] = inp1_zero_point
                    tmp_node = get_source_node(n.input[1], model.graph)
                    inp2_scale, inp2_zero_point = extract_scale_zero_point(tmp_node, initilizers_dict)
                    node_attributes['input2_zero_point'] = inp2_zero_point
                    tmp_node = get_dest_node(n.output[0], model.graph)
                    o_scale, o_zero_point = extract_scale_zero_point(tmp_node, initilizers_dict)
                    node_attributes['output_zero_point'] = o_zero_point
                    inp1_M0 = inp1_scale/o_scale
                    inp2_M0 = inp2_scale/o_scale
                    if fixed_scale is False:
                        inp1_M0, shift = decompose_float(inp1_M0, scale_bit_width)
                        node_attributes['input1_shift'] = shift
                        inp2_M0, shift = decompose_float(inp2_M0, scale_bit_width)
                        node_attributes['input2_shift'] = shift
                    else:
                        M0 = max(inp1_M0, inp2_M0)
                        bits, int_bits = calc_bit_width_fixed(M0, signed=False)
                        node_attributes['m0_bits'] = bits
                        node_attributes['m0_int_bits'] = int_bits
                    node_attributes['input1_M0'] = inp1_M0
                    node_attributes['input2_M0'] = inp2_M0
                ...
                if int_quant:
                    tmp_node = get_source_node(n.input[0], model.graph)
                    inp_scale, inp_zero_point = extract_scale_zero_point(tmp_node, initilizers_dict)
                    node_attributes['input_zero_point'] = inp_zero_point
                    tmp_node = get_dest_node(n.output[0], model.graph)
                    out_scale, out_zero_point = extract_scale_zero_point(tmp_node, initilizers_dict)
                    node_attributes['output_zero_point'] = out_zero_point
                    M0 = inp_scale/(out_scale*node_attributes['input_shape'][2]*node_attributes['input_shape'][3])
                    if fixed_scale is False:
                        M0, shift = decompose_float(M0, scale_bit_width)
                        node_attributes['shift'] = shift
                    else:
                        bits, int_bits = calc_bit_width_fixed(M0)
                        node_attributes['m0_bits'] = bits
                        node_attributes['m0_int_bits'] = int_bits
                    node_attributes['M0'] = M0
                self.cnt_avgpool += 1
                ...
                node_attributes['out_bits'] = self.default_bits
                node_attributes['out_int_bits'] = self.default_int_bits
                if int_quant and fixed_scale:
                    node_attributes['m_zp'] = []  # A list that contains the scale and zero point of each input to keep them in the same order and with out adding too many keys to the dictionary this is also dependent off the number of inputs
                    tmp_node = get_dest_node(n.output[0], model.graph)
                    o_scale, o_zero_point = extract_scale_zero_point(tmp_node, initilizers_dict)
                    max_m, max_zp = 0, 0
                    for inp in n.input:
                        tmp_node = get_source_node(inp, model.graph)
                        i_scale, i_zero_point = extract_scale_zero_point(tmp_node, initilizers_dict)
                        m = i_scale/o_scale
                        zp = -i_zero_point * m + o_zero_point
                        node_attributes['m_zp'].append((m,zp))
                        max_m = max(max_m, abs(m))
                        max_zp = max(max_zp, abs(zp))
                    node_attributes['m_bits'], node_attributes['m_int_bits'] = calc_bit_width_fixed(max_m)
                    node_attributes['zero_point_bits'], node_attributes['zero_point_int_bits'] = calc_bit_width_fixed(max_zp, signed=True)
                self.cnt_concat += 1
            else:
                print(f"NetworkHLS: onnx_to_networkx: unsupported layer: {n.op_type}")
                node_attributes['op_type'] = n.op_type
            ...
            if n.op_type in ['Conv', 'Gemm', 'MatMul', 'ConvTranspose']:
                if int_quant:
                    weight_node = get_source_node(n.input[1], model.graph)
                    weight, w_scale, w_zero_point = extract_val_scale_zero_point(weight_node, initilizers_dict)
                    weight = weight - w_zero_point
                    if n.op_type in ['Conv', 'ConvTranspose']:
                        node_attributes['weight'] = np.copy(weight)
                    else:
                        node_attributes['weight'] = np.copy(np.transpose(weight))
                    if len(n.input) == 3:
                        tmp_node = get_source_node(n.input[2], model.graph)
                        bias, _, _ = extract_val_scale_zero_point(tmp_node, initilizers_dict)
                        node_attributes['bias'] = bias
                    else:
                        node_attributes['bias'] = None
                    tmp_node = get_source_node(n.input[0], model.graph)
                    i_scale, i_zero_point = extract_scale_zero_point(tmp_node, initilizers_dict)
                    node_attributes['input_zero_point'] = i_zero_point
                    tmp_node = get_dest_node(n.output[0], model.graph)
                    o_scale, o_zero_point = extract_scale_zero_point(tmp_node, initilizers_dict)
                    node_attributes['output_zero_point'] = o_zero_point
                    M0 = (i_scale * w_scale) /o_scale
                    if fixed_scale is False:
                        M0,shift = decompose_float(M0, scale_bit_width)
                        node_attributes['shift'] = shift
                    else:
        ...
        attribute = {}
        attribute['depth'] = self.default_itermediate_streams_depth
                ...
                for dest in processed_nodes:
                    if out in model.graph.node[dest].input:
                        if dest in nx_graph.nodes():
                            ...
                            raise ValueError(f"NetworkHLS: onnx_to_networkx: Destination node {dest} not found in the graph")
        ...
        for idx in nx_graph.nodes():
            mapping[idx] = NodeHLS(nx_graph.nodes[idx], fixed_scale=fixed_scale)
            nx_graph.nodes[idx].clear()
        nx_graph = nx.relabel_nodes(nx_graph, mapping)

        return nx_graph

    def generate_config_hpp(self, nx_graph, config_hpp_path=None, scale_bit_width=32):
        ...
        for i, node in enumerate(tqdm(nx_graph.nodes())):
            if len(list(nx_graph.predecessors(node))) == 0:
                ...
                macros += f"const int num_packed_inputs = {math.ceil((node.in_bits * node.input_accesses) / self.input_mem_width)};\n"  # TODO: Update to handel multiple top levels
                if self.dtype is Dtypes.INT:
                    macros += f"#define {self._input_scale:70}{self.input_scale}\n"
                    macros += f"#define {self._input_zero_point:70}{self.input_zero_point}\n"
                    macros += f"#define {self._scale_bit_width:70}{scale_bit_width}\n"
                    macros += f"#define {self._act_min_value:70}{DefaultValues.min_act_value}\n"
                    macros += f"#define {self._act_max_value:70}{DefaultValues.max_act_value}\n"
                if any(op in node.op_type for op in [SupportedNodes.conv, SupportedNodes.tr_conv]):
                ...
                if self.dtype is Dtypes.INT:
                    macros += f"typedef ap_int<{self._input_bits}> {self._input_dtype};\n"
                else:
            ...
            if len(list(nx_graph.successors(node))) == 0:
                macros += f"const int num_packed_outputs = {math.ceil((node.out_bits * node.output_accesses) / self.output_mem_width)};\n"  # TODO: Update to handel multiple top levels
                ...
                if self.dtype is Dtypes.INT:
                    macros += f"typedef ap_int<{_output_bits}> {_output_dtype};\n"
                else:
    ...
    def find_concat_merge(self, nx_graph, split):
        '''
            The function finds the concat or merge node that is a successor of the split node.
            Even if the split node is not a direct predecessor of the concat or merge node, the function will find it.
            TODO: The current implementation can go creazy if the model is really complex
        '''
        successors = list(nx_graph.successors(split))
        target = None
        for succ in successors:
            if SupportedNodes.concat in succ.op_type or SupportedNodes.merge in succ.op_type:
                target = succ
                break
        if target is None:
            for succ in successors:
                target = self.find_concat_merge(nx_graph, succ)
                if target is not None:
                    break
        return target

    def set_fifo_depth(self, nx_graph, max_depth, set_to_max, fifos_size_percentage):
        '''
            The function sets the FIFO depth for the split merge and concat nodes in the graph.
            nx_graph: the graph to set the FIFO depth
            max_depth: if the FIFO depth is greater than this value, the FIFO will be set to off-chip
            set_to_max: if True, the FIFO depth will be set to the maximum value
            fifos_size_percentage: the percentage of the maximum FIFO size to be used
        '''
        ...
        for node in nx_graph.nodes():
            if SupportedNodes.split in node.op_type:
                ...
                nodes_path_depth = []
                edges_path_depth = []
                pair = list(nx_graph.successors(split))
                globalavgpool_in_succ = False
                for succ_node in pair:
                    if SupportedNodes.globalavgpool in succ_node.op_type: #  This covers the ConvEMA node in Yolo
                        pair.remove(succ_node)
                        globalavgpool_in_succ = True
                if globalavgpool_in_succ:
                    split_output_size = math.prod(split.output_shape)
                    off_chip = split_output_size > max_depth
                    for succ_node in pair:
                        nx_graph.edges[split, succ_node]['depth'] = split_output_size
                        nx_graph.edges[split, succ_node]['mapping'] = self.off_chip if off_chip else self.on_chip
                        if off_chip:
                            split.output_mem_width = 32
                            succ_node.input_mem_width = 32
                else:
                    merge = self.find_concat_merge(nx_graph, split)
                    if merge is None:
                        raise ValueError("No merge or concat node found in the pair.")
                    shortest_path = nx.shortest_path(nx_graph, split, merge)
                    all_simple_paths = list(nx.all_simple_paths(nx_graph, split, merge))
                    if len(set([len(path) for path in all_simple_paths])) == 1:
                        continue
                        # if all simple paths have the same length, then the nodes are running in parallel and it's not the traditional skip connection setup
                        # There still the case that one of the parallel nodes is faster than the other, which requires FIFOs depth adjustment
                        # TODO: Add support for this case
                    if set_to_max:
                        fifo_depth = math.prod(split.output_shape)
                    elif fifos_size_percentage is not None:
                        fifo_depth = int(math.prod(split.output_shape) * fifos_size_percentage)
                    else:
                    ...
                    if nx_graph.edges[shortest_path[-2], merge]['depth'] > max_depth:
                        nx_graph.edges[shortest_path[-2], merge]['mapping'] = self.off_chip

        return nx_graph

    def set_parallelism(self, nx_graph, chain_rule=True):
        '''
            The function sets SIMD and PE depending on:
                1. target_latency_s - target latency in seconds, which is the reciprocal of FPS
         '''
        print(f'Setting parallelism to achieve target latency of {self.target_latency_s}s which is {self.target_latency_cc} cc..')
        for node in nx_graph.nodes():
            node_latency = node.get_latency_cc_node()
            if max(node_latency) > self.target_latency_cc:
                if node_latency[0] > node_latency[1]:  # Buffer is the bottleneck
                    ...
                    constant = None
                paralleism_factor = math.ceil(max(node_latency)/self.target_latency_cc)
                ...
                if len(list(nx_graph.predecessors(node))) == 0:
                    edge_simd_pe = 'simd'
                    interface_width = self.input_mem_width
                elif len(list(nx_graph.successors(node))) == 0:
                    edge_simd_pe = 'pe'
                    interface_width = self.output_mem_width
                else:
                    edge_simd_pe = None
                    interface_width = None
                node.simd, node.pe = find_simd_pe(num_inputs, num_outputs, paralleism_factor, constant, edge_simd_pe, interface_width)
        if chain_rule:
            for node in nx_graph.nodes():
                if SupportedNodes.split in node.op_type or SupportedNodes.merge in node.op_type:
                    ...
                    for succ in successors:
                        if max_value <= succ.max_simd:
                            succ.simd = max_value
                        else:
                            print(f"\033[91mChain rule could not be applied between {node.name} and {succ.name}\033[0m")
                    for pred in predecessors:
                        if max_value <= pred.max_pe:
                            pred.pe = max_value
                        else:
                            print(f"\033[91mChain rule could not be applied between {node.name} and {pred.name}\033[0m")
                    node.pe = max_value
                    node.simd = max_value
                else:
                    ...
                    if len(node_successors) > 0:
                        ...
                        node.pe = max_value if max_value <= node.max_pe else node.pe
                        for succ in node_successors:
                            if node.pe <= succ.max_simd:
                                succ.simd = node.pe
                            else:
                                print(f"\033[91mChain rule could not be applied between {node.name} and {succ.name} due to different weight shapes: {node.weight.shape} vs {succ.weight.shape}\033[0m")
        ...
        for node in sorted_nodes:
            ...
            if node.op_type == SupportedNodes.merge:
                for predecessor in predecessors:
                    if nx_graph.edges[predecessor, node]['depth'] == self.default_itermediate_streams_depth:
                        # The functions expects that 'direct' edge has the default depth, while 'fifo' edge has a depth > default
                        # The Merge inherits the number of accesses from the 'direct' edge
                        node.input_accesses = predecessor.output_accesses
                        node.simd = predecessor.pe
                        node.pe = predecessor.pe
                        node.output_accesses = predecessor.output_accesses
                ...
                node.simd = predecessors[0].pe
                node.pe = predecessors[0].pe
            elif SupportedNodes.concat in node.op_type:
                # For concat layer, output access is the sum of input access of all predecessors
                node.input_accesses = predecessors[0].output_accesses
                node.output_accesses = int(sum([predecessor.output_accesses for predecessor in predecessors]))
                node.simd = predecessors[0].pe
                node.pe = predecessors[0].pe
            elif SupportedNodes.sdwc in node.op_type:
                predecessor = predecessors[0]
                assert predecessor.output_accesses % predecessor.output_stream_width == 0, f"Output accesses of {predecessor.name} is not divisible by output stream width {predecessor.output_accesses} % {predecessor.output_stream_width} != 0"
                node.output_accesses = predecessor.output_accesses * predecessor.output_stream_width // node.output_stream_width
            elif not any(i in node.op_type for i in [SupportedNodes.conv, SupportedNodes.tr_conv, SupportedNodes.linear, SupportedNodes.expand, SupportedNodes.globalavgpool, SupportedNodes.sdwc]):
                # Only Merge layer has more than one predecessor
                # All layers have pe = simd axcept Conv and Linear
                node.input_accesses = predecessors[0].output_accesses
                node.output_accesses = predecessors[0].output_accesses
                node.simd = predecessors[0].pe
                node.pe = predecessors[0].pe
        ...
        for node in sorted_nodes:
            if node.op_type == SupportedNodes.merge:
                ...
                for predecessor in list(nx_graph.predecessors(merge)):
                    if nx_graph.edges[predecessor, merge]['depth'] == self.default_itermediate_streams_depth:
                        # The functions expects that 'direct' edge has the default depth, while 'fifo' edge has a depth > default
                        # The Merge inherits the number of accesses from the 'direct' edge
    ...
    def create_single_subgraph_default(self, nx_graph, criteria=None):
                ...
                if len(list(nx_graph.predecessors(node))) == 0:
                    ...
                    for predecessor in sorted_predecessors:
                        ...
                        if nx_graph.edges[predecessor, node]['mapping'] == self.off_chip:
                            ...
                            input_mem_name = f"{name}_input_mem"
                            if node.op_type == SupportedNodes.merge and nx_graph.edges[predecessor, node]['depth'] != self.default_itermediate_streams_depth:
            ...
            for node in sorted_nodes:
                ...
                if len(list(nx_graph.predecessors(node))) == 0:
                    ...
                    if self.input_mem_width != node.input_stream_width:
                        ...
                        on_chip_streams += f"#pragma HLS STREAM variable={m2s_name} depth={self.default_itermediate_streams_depth}\n"
                        ...
                        on_chip_streams += f"#pragma HLS STREAM variable={input_stream_name} depth={self.default_itermediate_streams_depth}\n"
                    ...
                    # Get predecessor with more ancestors first. Predecessor with less ancestors is a shortcut
                    sorted_predecessors = sorted(nx_graph.predecessors(node), key=lambda node: nx.ancestors(nx_graph, node), reverse=False)  # This was originally True, if you want to set it to True, you need to modify the parts that handel Add_streams and concat streams to correctly order the streams and the corresonponding scale and zero points
                    for predecessor in sorted_predecessors:
                    #for predecessor in list(nx_graph.predecessors(node)):
                        name = f"{predecessor.name.lower()}_{node.name.lower()}"
                        if nx_graph.edges[predecessor, node]['mapping'] == self.off_chip:
                            ...
                            # input_mem_width is in_bits_aligned packed to 32, 64 or 128
                            if node.input_mem_width not in [32, 64, 128]:
                                print(f'Width of the output memory port should be 32, 64 or 128, got {node.input_mem_width}, between {node.name} and {successor.name}, replacing with {self.default_input_mem_width}')
                                node.input_mem_width = self.default_input_mem_width
                            ...
                            if subgraph.has_node(predecessor) is not True:
                                ...
                                if predecessor.output_stream_width != input_stream_width:
                                    ...
                                    on_chip_streams += f"#pragma HLS STREAM variable={stream_width_convertor_name} depth={self.default_itermediate_streams_depth}\n"
                ...
                if len(list(nx_graph.successors(node))) == 0:
                    ...
                    if self.output_mem_width != node.output_stream_width:
                        ...
                        on_chip_streams += f"#pragma HLS STREAM variable={output_stream_name} depth={self.default_itermediate_streams_depth}\n"
                        ...
                        on_chip_streams += f"#pragma HLS STREAM variable={s2m_name} depth={self.default_itermediate_streams_depth}\n"
                    ...
                    #for successor in list(nx_graph.successors(node)):
                        ...
                        if nx_graph.edges[node, successor]['mapping'] == self.off_chip:
                            ...
                            if node.output_mem_width not in [32, 64, 128]:
                                print(f'Width of the output memory port should be 32, 64 or 128, got {node.output_mem_width}, between {node.name} and {successor.name}, replacing with {self.default_output_mem_width}')
                                node.output_mem_width = self.default_output_mem_width
